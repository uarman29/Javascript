...
<html>
	<head> 
		...
		<?php include "updater.php"?>
			...
			function initialize()
			{
				...
				attackElement=document.getElementById("attack");
				soundElement=document.getElementById("sound");
				imageElement=document.getElementById("image");
				titleElement.innerHTML="<?php getTime("Name","4");?>";
				messageElement.innerHTML="<?php getTime("Time","4");?>";
				Health=<?php getValue("Health");?>
				Oxygen=<?php getValue("Oxygen");?>
				BodyTemp=<?php getValue("BodyTemp");?>
				Temp=<?php getValue("Temp");?>
				Data=<?php getTable();?>;
				obtained=<?php checkPeice("p5");?>;
				checkPeice();
				fillTable();
				setTimeout(sandstorm,1000);
			...
			function sandstorm()
			{
				attackElement.style.zIndex="-1";
				statusElement.innerHTML+="<br/>"+"SANDSTORM!!";
				statusElement.innerHTML+="<br/>"+"Sand is interferring with devices evacuate Immediatly!!!!";
				statusElement.innerHTML+="<br/>"+"Its hard to see the Peice";
				checkPeice();
				setInterval(checkPeice,4000);
				setInterval(checkHealth,850,"Yes");
			}
			function checkPeice()
			{
				statusElement.innerHTML+="<br/>"+"The sand is Blowing away the Peice";
				if(obtained[0][1]=="Yes")
				{
			...
			function checktemp()
			{
				if(<?php getTime("Temp","4");?>>=30&&<?php getTime("Temp","4");?><=125)
				{
					Temp=5;
					sendRequest("Temp",Temp);
				}
				if(<?php getTime("Temp","4");?><=30)
				{
					Temp=0;
					sendRequest("Temp",Temp);
				}
				if(<?php getTime("Temp","4");?>>=125)
				{
					Temp=10;
					sendRequest("Temp",Temp);
				}
	...
	<img id="back"src="Images/pyramid.jpg"/>
	<img id="attack"src="Images/sandstorm.png"/>
	<form id="myForm" method ="post" action="Portal.php">
		...
		<img id="image" style="position:absolute;height:3%;width:2%;"src="Images/p5.png" onmousedown="getItem('Yes');"onmouseup="getItem('No');"/>
	</form>
