...
<html>
	<head> 
		...
		<?php include "updater.php"?>
			...
			function initialize()
			{
				...
				titleElement.innerHTML="<?php getTime("Name","3");?>";
				messageElement.innerHTML="<?php getTime("Time","3");?>";
				Health=<?php getValue("Health");?>
				Oxygen=<?php getValue("Oxygen");?>
				BodyTemp=<?php getValue("BodyTemp");?>
				Temp=<?php getValue("Temp");?>
				Data=<?php getTable();?>;
				obtained=<?php checkPeice("p4");?>;
				checkPeice();
				fillTable();
				setInterval(randomAttack,getRandomInteger(3000,5000));
				Oxygenupdate=setInterval(checkOxygen,3000);
				if("<?php getTime("Danger","2");?>"=="No")
				{
			...
			function randomAttack()
			{
				...
				statusElement.innerHTML+="<br/>"+"MAMMOTH ATTACK";
				checkHealth("Yes");
				setTimeout(clear,2000);
			}
			function clear()
			{
				attackElement.style.zIndex="-2";
			}
			function checkPeice()
			{
				if(obtained[0][1]=="Yes")
			...
			function checktemp()
			{
				if(<?php getTime("Temp","3");?>>=30&&<?php getTime("Temp","3");?><=125)
				{
					Temp=5;
					sendRequest("Temp",Temp);
				}
				if(<?php getTime("Temp","3");?><=30)
				{
					Temp=0;
					sendRequest("Temp",Temp);
				}
				if(<?php getTime("Temp","3");?>>=125)
				{
					Temp=10;
					sendRequest("Temp",Temp);
				}
	...
	<img id="back"src="Images/StoneAgeBack.jpg"/>
	<img id="attack"src="Images/StoneAgeAttack.png"/>
	<form id="myForm" method ="post" action="Portal.php">
		...
		<img id="image" style="position:absolute;height:3%;width:2%;"src="Images/p4.png" onmousedown="getItem('Yes');"onmouseup="getItem('No');"/>
	</form>
